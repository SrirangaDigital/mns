...
open(IN,"mns_special_books.xml") or die "can't open mns_special_books.xml\n";

my $dbh=DBI->connect("DBI:mysql:database=$db;host=$host","$usr","$pwd");

$sth1=$dbh->prepare("CREATE TABLE special_books_list(
book_id varchar(4), 
level int(2),
title varchar(1000),
authid varchar(200),
...
sub insert_to_db()
{
	...
	$sth2=$dbh->prepare("insert into special_books_list values('$book_id','$level','$title','$authid','$authors','$page','$page_end','$edition','$volume','$part','$type','$year','$month','')");
	$sth2->execute();
	$sth2->finish();
}
