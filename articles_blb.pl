...
open(IN,"mns_blb.xml") or die "can't open mns_blb.xml\n";

my $dbh=DBI->connect("DBI:mysql:database=$db;host=$host","$usr","$pwd");

#vnum, number, month, year, title, feature, authid, page, 

$sth11r=$dbh->prepare("CREATE TABLE article_blb(title varchar(500), 
authid varchar(200),
...
while($line)
{
	...
	elsif($line =~ /<page>(.*)<\/page>/)
	{
		...
		{
			$count++;
			$id = "blb_" . $volume . "_" . $part . "_" . $page . "_" . $page_end . "_" . $count; 
		}
		else
		{
			$id = "blb_" . $volume . "_" . $part . "_" . $page . "_" . $page_end . "_0";
			$count = 0;		
		}
		$prev_pages = $pages;		
	}	
...
sub insert_article()
{
	...
	$sth1=$dbh->prepare("insert into article_blb values('$title','$authids','$author_name','$featid','$page','$page_end','$plates','$volume','$part','$year','$month','$id')");
...
sub get_featid()
{
	...
	$sth=$dbh->prepare("select featid from feature_blb where feat_name='$feature'");
	$sth->execute();
