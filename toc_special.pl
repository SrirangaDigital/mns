...
open(IN,"mns_special_toc.xml") or die "can't open mns_special_toc.xml\n";

my $dbh=DBI->connect("DBI:mysql:database=$db;host=$host","$usr","$pwd");

$sth1=$dbh->prepare("CREATE TABLE special_book_toc(book_id varchar(4), 
level int(2),
title varchar(1000),
page varchar(10),
type varchar(4),
...
sub insert_to_db()
{
	...
	$sth2=$dbh->prepare("insert into special_book_toc values('$book_id','$level','$title','$page','$type','')");
	$sth2->execute();
	$sth2->finish();
}
